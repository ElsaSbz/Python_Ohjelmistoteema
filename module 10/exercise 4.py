...
class Race:
    def __init__(self, name, distance, participated_cars):
        self.name = name
        self.distance = distance
        self.cars = participated_cars

    def hour_passes(self):
        for car in self.cars:
            # Generate a random speed between -10 km/h and +15 km/h
    ...
    def print_status(self):
        print(f"\nRace: {self.name}")
        print(
            f"{'Registration Number':<20}{'Maximum Speed (km/h)':<20}{'Current Speed (km/h)':<20}{'Travelled Distance (km)':<25}")
        print("=" * 85)
        for car in self.cars:
            print(
                f"{car.registration_number:<20}{car.maximum_speed:<20}{car.current_speed:<20}{car.travelled_distance:<25}")
...
cars = []
for i in range(1, 11):
    maximum_speed = random.randint(100, 200)  # Random max speed between 100 and 200 km/h
    registration_number = f"ABC-{i}"
    cars.append(Cars(registration_number, maximum_speed))
